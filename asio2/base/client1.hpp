...
#ifndef __ASIO2_CLIENT1_HPP__
#define __ASIO2_CLIENT1_HPP__
...
namespace asio2::detail
{
	...
	public:
		...
		using self  = client1_impl_t<derived_t, args_t>;
		...
		explicit client1_impl_t(
			...
			, socket_cp           <derived_t, args_t>(io, std::forward<Args>(args)...)
			, connect_cp          <derived_t, args_t>()
			, disconnect_cp       <derived_t, args_t>()
			, local_endpoint_cp   <derived_t, args_t>()
			, reconnect_timer_cp  <derived_t, args_t>(io)
			, user_timer_cp       <derived_t, args_t>()
			, connect_timeout_cp  <derived_t, args_t>(io)
