...
#ifndef __ASIO_REDIS_REDIS_REPLY_PARSER_HPP__
#define __ASIO_REDIS_REDIS_REPLY_PARSER_HPP__

#include <stack>
#include <vector>
#include <utility>

#include <asio/redis/impl/reply.hpp>
...
public:
    ...
    enum ParseResult {
     ...
     reply& result(){ return reply_; }
    const reply& result()const { return reply_; }
protected:
    void reset()
    {
...
std::pair<size_t, reply_parser::ParseResult> reply_parser::parse(const char *ptr, size_t size)
{
    return reply_parser::parse_chunk(ptr, size);
}

std::pair<size_t, reply_parser::ParseResult> reply_parser::parse_chunk(const char *ptr, size_t size)
{
    size_t position = 0;
    State state = Start;
    ...
    while(position < size)
    {
            ...
            case ArraySizeLF:
                if( c == '\n' )
                {
                    ...
                    else if( arraySize == 0 )
                    {
                        ...
                        reply_.emplace<std::vector<reply>>(std::move(array));
                    }
                    else if( arraySize < 0 )
                    {
                        std::stack<State>().swap(states_);
        ...
        if (state == Start)
        {
            if (!array_sizes_.empty())
            {
                ...
                array_reply_.top().get<reply::array_type>().emplace_back(std::move(reply_));

                while(!array_sizes_.empty() && --array_sizes_.top() == 0)
                {
                    array_sizes_.pop();
                    // std::cout<<"array ok:"<<array_reply_.top().get<reply::array_type>().size()<<std::endl;
                    reply_ = std::move(array_reply_.top());
                    // std::cout<<"array ok:"<<reply_.get<reply::array_type>().size()<<std::endl;
                    array_reply_.pop();
 ...
 */
long int reply_parser::buf_to_long(const char *str, size_t size)
{
    long int value = 0;
    bool sign = false;
...
#endif // __ASIO_REDIS_REDIS_REPLY_PARSER_HPP__
